...
EmployeeManager::~EmployeeManager() { mDbConnection->disconnect(); }
...
EmployeeManager::vectorToMap(std::vector<Employee> const &employees) const {
...
std::map<std::string, float>
EmployeeManager::getSalariesGreater(float low) const {
  return vectorToMap(mDbConnection->getSalariesRange(low));
}
std::map<std::string, float>
EmployeeManager::getSalariesBetween(float low, float high) const {
