...
TEST(TestEmployeeManager, TestGetSalary) {
  ...
  EXPECT_CALL(dbConnection, connect());
  EXPECT_CALL(dbConnection, disconnect());
  EXPECT_CALL(dbConnection, getSalary(testing::_))
      .Times(1)
...
TEST(TestEmployeeManager, TestGetSalaryInRange) {
  ...
  EXPECT_CALL(dbConnection, connect());
  EXPECT_CALL(dbConnection, disconnect());
  EXPECT_CALL(dbConnection, getSalariesRange(low, high))
      .WillOnce(testing::Return(returnedVector));

  EmployeeManager employeeManager(&dbConnection);

  std::map<std::string, float> returnedMap =
      employeeManager.getSalariesBetween(low, high);

  for (auto it = returnedMap.begin(); it != returnedMap.end(); ++it) {
    std::cout << it->first << " " << it->second << '\n';
    ASSERT_THAT(it->second, testing::AnyOf(testing::Gt(low)));
  }
}

// TEST(TestEmployeeManager, TestCallback) {
//   MockDatabaseConnection dbConnection("myConnection");
//   testing::MockFunction<void(int, int)> mocfunction;
//   dbConnection.setOnConnect(mocfunction.AsStdFunction());

//   EXPECT_CALL(mocfunction, Call(testing::_, testing::_));
